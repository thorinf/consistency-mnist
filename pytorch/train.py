...
def train():
    ...
    p_groups = [
        {"params": [p for p in score_model.parameters() if p.dim() != 1], "weight_decay": args.weight_decay},
        {"params": [p for p in score_model.parameters() if p.dim() == 1], "weight_decay": 0.0},
    ]

    optim = torch.optim.AdamW(p_groups, lr=args.learning_rate)
    ...
    for ep in range(checkpoint.get("epochs", 0), args.epochs):
        ...
        for idx, (data, labels) in enumerate(pbar):
            ...
            loss = consistency.compute_loss(
                model=score_model, target_model=ema_score_model, x_target=data, num_scales=N, labels=labels
            )
        ...
        with torch.no_grad():
            ...
            x_list = consistency.sample(model=ema_score_model, x_start=x_start, ts=ts, return_all=True, labels=labels)
